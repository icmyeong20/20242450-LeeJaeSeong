...
int trainAndp(int *length, int *MDSs, int *p) { // 기차, 마동석 체력, 확률 입력
...
int train1st(int length) { // 첫번째 기차 출력
    for (int i = 0; i < 3; i++) {
        if (i == 0 || i == 2) {
...
int trainT(length, C, Z, M) { // 반복 기차 출력
    for (int i = 0; i < 3; i++) {
        if (i == 0 || i == 2) {
...
int move(int Rc, int p, int Md) { // 확률 이동
    if (rand() % 100 < p) {
        return Rc + Md;
    }
    return Rc;
...
int moveT(T, C, Z, Cz, Zz, Ca, Caz, Ma, Maz) { // 시민,좀비 상태
    if (Cz == C) {
        if (AGGRO_MIN < Ca) {
            Ca--;
        }
...
int gogo(length, p) { // 시민, 좀비, 마동석 움직임 여부, 게임 승패 출력
